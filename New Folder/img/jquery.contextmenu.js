    ...
    op = {
        ...
        create: function(opt, root) {
            ...
            $.each(opt.items, function(key, item){
                var $t = $('<a class="context-menu-item ' + (item.className || "") +'"></a>'),
                    $label = null,
                    $input = null;
                ...
                if (typeof item == "string") {
                    ...
                    if (item.icon) {
						...
						$t.css("background-image", "url('img/"+item.icon+"')");
                    }
					if (item.href) {
						$t.attr("href", item.href);
					}
					if (item.onclick) {
						$t.attr("onclick", item.onclick);
					}
					if (item.target) {
						$t.attr("target", item.target);
					}
                }
